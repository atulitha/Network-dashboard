treeJSON = d3.json("flare.json", function(error, treeData) {
    ...
    var nodeWidth = 300;
    var nodeHeight = 50;
    var horizontalSeparationBetweenNodes =30;
	...
	console.log("welcome");
    var multiParentsread=JSON.parse('{{ mulparents | tojson | safe}}'); ;
   
    
    var arrayLength = multiParentsread.length;
    console.log("hello");
    var multiParents=[]
    for(i=0;i<multiParentsread.length;i++){
        l=multiParentsread[i];

        console.log(l);
        var p=tree.nodes(root).filter(function(d){return d['name']==l['parent'];})[0];
        var c=tree.nodes(root).filter(function(d){return d['name']==l['child'];})[0];
        multiParents.push({
            parent:p,
            child:c
        });
	...
	multiParents.forEach(function(multiPair) {
            ...
            .attr("class", "additionalParentLink")
                .attr("d", function() {
                    ...
                    console.log(multiPair.parent.x0);
                    return diagonal({
                        source: oSource,
                        target: oTarget
                    });
